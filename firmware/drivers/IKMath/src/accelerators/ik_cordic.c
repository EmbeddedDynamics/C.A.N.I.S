...
ik_result_t ik_cordic_start(ik_cordic_h handle)
{
    ...
    handle->state = CORDIC_STATE_RUNNING;
    ...
    ik_vector3f_t* out)
{
    ...
    uint8_t job_id = IK_CORDIC_INVALID_JOB_ID;
    ...
    if (format == IK_CORDIC_FORMAT_VECTOR) {
        job->fmt = handle->config.vec_fmt;
    } else {
        job->fmt = handle->config.unit_fmt;
    }
...
)
{
    ...
    job->coord = coord;
    job->operation = IK_CORDIC_OPERATION_ROTATION;
    job->input = *vec_in;
    job->output = out;
...
ik_result_t ik_cordic_queue_job(ik_cordic_h h, uint8_t job_id)
{
    ...
    h->jobs[job_id].state = CORDIC_JOB_STATE_WAITING;
    return IK_RESULT_OK;
}
